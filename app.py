...
def _get_csv_file(file_key_path_src=FILE_KEY_PATH_SRC, aws_s3_bucket_src=AWS_S3_BUCKET_SRC, use_limitation=True):
    ...
    if status == 200:
        ...
        if data_length and use_limitation:
            # count = 0
            # for data in pd.read_csv(response.get("Body"), chunksize=1000):
            #     count += 1                          # counting the number of chunks
            #     lastlen = len(data)                 # finding the length of last chunk
            # actual_length = (count*1000 + lastlen - 1000) # length of total file
            # skip_rows = actual_length - data_length if actual_length > data_length else 0
            # print(skip_rows)
            # if skip_rows:
            #     df = pd.read_csv(response.get("Body"),  skiprows=skip_rows, nrows=data_length) 
            # else:
            #     df = pd.read_csv(response.get("Body"), nrows=data_length)
            df = pd.read_csv(response.get("Body"), nrows=data_length)
        else:
...
mregularseasons = _get_csv_file("MDataFiles_Stage2/MRegularSeasonCompactResults.csv", AWS_S3_BUCKET_SRC, False)
...
meventsteams_transformed_clone = meventsteams_transformed
...
def _get_seasons():
    ...
    seasons = [2015, 2016, 2017, 2018, 2019]# limited 
    return seasons
...
def _get_events_statistics(season):    
    ...
    stats = html.Div(className="level box", children=[
                    html.Div(className="level-item", children=[
                       html.P("No data provided for selected season")
                    ])
                ])
    if season and season in used_seasons:
        ...
        is_all_time = True
    if len(mevents_statistics):
        if 'Event Type' not in mevents_statistics.keys():
            mevents_statistics['Event Type'] = mevents_statistics['EventType']
...
def _get_player_events(player_id, season=selected_season): 
    ...
    if 'Event Type' not in mevents_most.keys():
        mevents_most['Event Type'] = mevents_most['EventType']
...
def _get_player_stats_box():
    return [
                html.Div(className="column", children=[
                    html.Div(className="box", children=[
                        html.Span(className="", children="Most move"),
                        html.H3(className="has-text-weight-bold", id="player-stats-1-h3", children="-"),
                    ])
                ]),
                html.Div(className="column", children=[
                    html.Div(className="box", children=[
                        html.Span(className="", children="Least move"),
                        html.H3(className="has-text-weight-bold", id="player-stats-2-h3", children="-"),
                    ])
                ]),
                html.Div(className="column", children=[
                    html.Div(className="box", children=[
...
@dash_app.callback(
    ...
    Output(component_id='most-championship-wins-chart', component_property='figure'),
    ...
    Output(component_id='player-stats-box', component_property='children'),
...
def _on_season_dropdown_change(value, children):
    print(value, transformed_seasons)
    if value:
        selected_season = value
        if value in event_seasons and value not in transformed_seasons:
            meventsteams_transformed = _get_mevents(value)
        else:
            meventsteams_transformed = meventsteams_transformed_clone
        _season_text = f'Season {value}'
        _team_options = _get_teams(value)
        _most_tournament_wins_chart = _get_most_tournament_wins_chart(value)
        _most_championship_wins_chart = _get_most_championship_wins_chart(value)
        _events_statistics = _get_events_statistics(value)  
        _predictions = _get_predictions_list(value)
        # _area_bar_chart = _get_area_bar_chart(value)
        _stats_box = _get_player_stats_box()
        return [_season_text, _most_tournament_wins_chart, _most_championship_wins_chart, _events_statistics, _predictions, _stats_box,  _team_options, '', '']
    else: 
...
def _on_player_dropdown_change(value, selected_team):
    if not value:
        ...
        if len(most_event):
            ...
            most_event = "Unknown"
        least_event = (player_events.sort_values(by="Count", ascending=True)['Event Type'].values.tolist()[:5])
        if len(least_event):
...
_body_stats_row = html.Div(className="columns is-multiline", children=[
    ...
    html.Div(className="column", children=[
        html.Div(className="box", children=[
            ...
            html.Br(),
            html.Div(id="event-statistics-list", children=_get_events_statistics(selected_season))
        ]),
    ])
])
...
_body_custom_row = html.Div(className="columns is-multiline", children=[
    html.Div(className="column", children=[
        html.Div(className="box", children=[
            ...
            html.Br(),
            html.Div(id="top-winner-predictions-list", className="", children=_get_predictions_list(selected_season)),
        ]),
    ]),
    html.Div(className="column", children=[
        html.Div(className="box", children=[
            ...
            html.Br(),
            html.Div(id="player-stats-box", className="columns", children=[])
        ]),
    ])
])
...
def _debug(year):
    print("================================================== DEBUGGING ========================================")
    print(_get_teams(year))
    print(_get_most_tournament_wins_chart(year))
    print(_get_players('1100'))
    print(_get_events_statistics(year))
    print(_get_player_events(1000, 2000))
    print(_get_predictions_list(year))
    print(_on_season_dropdown_change(year, []))
